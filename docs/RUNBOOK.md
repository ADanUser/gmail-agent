...
- Свободный тариф OpenRouter имеет строгие лимиты запросов — проект
  переключён на Groq (`llama-3.1-8b-instant`) как более стабильный
...
| `openai.RateLimitError: Error code: 429 ... tokens per day (TPD)` в CI или при локальном прогоне | Исчерпан дневной лимит токенов бесплатного тарифа Groq (100 000 токенов/день). Часто возникает при многократных ручных прогонах `eval_runner.py`/`security_tests.py` в течение дня плюс автоматические прогоны CI на каждый push | Не баг кода — подождать сброса дневного лимита (обычно раз в сутки) и нажать "Re-run jobs" в GitHub Actions, либо перейти на платный тариф Groq при необходимости частых прогонов |
